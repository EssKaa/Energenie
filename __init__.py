...
class EGPM(eg.PluginBase):
    ...
    def Configure(self, ip_address="", password="", event=True):
        ...
        labelIpAddress = wx.StaticText(panel, label="Device IP", pos=(x_start, y_start + label_padding + (i * y_padding)))
        textControlIpAddress = wx.TextCtrl(panel, -1, ip_address, (x_start + (x_padding * 2), y_start + (i * y_padding)), (150, -1))

        i += 1
        labelPassword = wx.StaticText(panel, label="Password", pos=(x_start, y_start + label_padding + (i * y_padding)))
        textControlPassword = wx.TextCtrl(panel, -1, password, (x_start + (x_padding * 2), y_start + (i * y_padding)), (150, -1))

        i += 1
        labelEvent = wx.StaticText(panel, label="Event on EG command?", pos=(x_start, y_start + label_padding + (i * y_padding)))
        checkBoxEvent = wx.CheckBox(panel, -1, "", (x_start + (x_padding * 2), y_start + (i * y_padding+4)), (150, -1))
...
class SetSocketAOn(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=0, action=1)

class SetSocketAOff(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=0, action=0)

class SetSocketAToggle(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=0, action=2)

class SetSocketBOn(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=1, action=1)

class SetSocketBOff(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=1, action=0)

class SetSocketBToggle(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=1, action=2)

class SetSocketCOn(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=2, action=1)

class SetSocketCOff(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=2, action=0)

class SetSocketCToggle(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=2, action=2)

class SetSocketDOn(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=3, action=1)

class SetSocketDOff(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=3, action=0)

class SetSocketDToggle(ActionBase):
    def __call__(self):
        return self.egpmSetSocketStateWrapper(host=self.plugin.egpmIp, password=self.plugin.egpmPw, socketNr=3, action=2)
